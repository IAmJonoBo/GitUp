...
import { StepBasics, StepType, StepCI, StepStack, StepQuality, StepDocs, StepSecurity, StepReview, StepGitHub } from './StepForms';
  ...
  return (
    <div className="flex flex-col h-full max-w-3xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
           <div>
               <h2 className="text-3xl font-bold tracking-tight text-white mb-1">{steps[step].title}</h2>
               <p className="text-zinc-500 text-sm">Step {step + 1} of {steps.length}</p>
           </div>
                ...
                <div className="flex items-center gap-1 bg-zinc-900 p-1 rounded-lg border border-white/5">
                        <button 
                            ...
                            className={cn(
                                ...
                                userMode === 'basic' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'
                            )}
                        >
                            Basic
                        </button>
                        <button 
                            ...
                            className={cn(
                                ...
                                userMode === 'power' ? 'bg-purple-900/30 text-purple-300 border border-purple-500/20 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'
                            )}
                        >
      ...
      <div className="flex items-center justify-between mb-10 relative px-1">
        ...
        {steps.map((s, i) => {
            ...
            return (
                <div key={s.id} className="flex flex-col items-center gap-2 relative bg-zinc-950 px-2 rounded-full group">
                    <button
                        ...
                        className={cn(
                            ...
                            // Active (Current)
                            isActive && "bg-zinc-900 text-white border border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.6)] scale-110",
                            
                            // Completed (Past)
                            isCompleted && "bg-zinc-900 text-emerald-400 border border-emerald-500/50 shadow-[0_0_10px_rgba(52,211,153,0.3)] cursor-pointer hover:bg-zinc-800 hover:border-emerald-400 hover:scale-105 hover:text-emerald-300",
                            
                            // Future (but visited/navigable - e.g. went back)
                            !isActive && !isCompleted && canNav && "bg-zinc-900 text-zinc-400 border border-zinc-700 cursor-pointer hover:border-zinc-500 hover:text-zinc-200 hover:bg-zinc-800",
                            
                            // Future (Locked)
                            !canNav && "bg-zinc-900 border border-zinc-800 text-zinc-600 cursor-not-allowed"
                        )}
                    ...
                    >
                        ...
                        <span className={cn(
                            ...
                            isActive ? "opacity-100 text-purple-300" : "opacity-0 group-hover:opacity-100 text-zinc-400"
                        )}>
                            {s.title}
                        </span>
                    </button>
