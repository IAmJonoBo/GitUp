...
const DEPENDENCY_FREQUENCIES: PlanConfig["security"]["dependencyUpdateFrequency"][] =
  ["daily", "weekly", "monthly"];
const ACTION_PERMISSION_OPTIONS: PlanConfig["github"]["actions"]["permissions"][] =
  ["all", "local", "none"];
const NOISE_BUDGET_PRESETS: NoiseBudgetPreset[] = ["low", "medium", "high"];
const GOVERNANCE_POSTURES: GovernancePosture[] = [
  "Relaxed",
  "Team Standard",
  "Strict",
];
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Prepare project for multiple languages & locales
              </div>
            </div>
          </div>
          <Switch checked={i18n} onCheckedChange={(v) => setValue("i18n", v)} />
        </div>
        ...
        {i18n && (
          <div className="p-3 bg-muted/50 border border-border rounded-lg text-[10px] text-muted-foreground -mt-2 animate-in slide-in-from-top-1">
            Will scaffold a <code className="text-foreground">locales</code>{" "}
            directory and configure the i18n routing middleware.
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <span
                className={cn(
                  ...
                  visibility === "public" ? "text-primary" : "text-foreground",
                )}
              >
                Public
              </span>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Open source. Visible to everyone on GitHub.
            </p>
          </div>
          <div
          ...
          >
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Restricted access. Only visible to you and your team.
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-[300px]">
                <p>
                  <strong>Polyrepo:</strong> One project per repository. Simple,
                  standard.
                </p>
                <p className="mt-2">
                  <strong>Monorepo:</strong> Multiple projects in one repo.
                  Great for shared code, but requires more tooling.
                </p>
              </TooltipContent>
            </Tooltip>
          </div>
...
export const StepType = () => {
  ...
  const types = [
    {
      ...
      desc: "Frontend or Full-stack app",
    },
    {
      id: ProjectType.SERVICE,
      icon: Server,
      label: "Backend Service",
      desc: "API, Worker, or Microservice",
    },
    {
      id: ProjectType.LIBRARY,
      icon: Library,
      label: "Library / Package",
      desc: "Reusable code for other apps",
    },
    {
      id: ProjectType.CLI,
      icon: Terminal,
    ...
    },
    {
      ...
      icon: Monitor,
      label: "Desktop App",
      desc: "Native desktop experiences",
    },
  ];
  ...
  const architectures: { id: Architecture; desc: string; rec: boolean }[] = [
    {
      id: "Standard",
      desc: "Simple, folder-by-type structure. Good for small apps.",
      rec: config.type === ProjectType.WEB,
    },
    {
      id: "Hexagonal",
      desc: "Ports & Adapters. Isolates domain logic from infrastructure.",
      rec: config.type === ProjectType.SERVICE,
    },
    {
      id: "Clean",
      desc: "Concentric layers (Entities, Use Cases). Maximizes testability.",
      rec: false,
    },
    {
      id: "MVC",
      desc: "Model-View-Controller. Classic pattern for separation of concerns.",
      rec: false,
    },
    {
      id: "Event-Driven",
      desc: "Async communication via events. Good for scalable distributed systems.",
      rec: false,
    },
    {
      id: "Vertical Slice",
      desc: "Group by feature rather than technical layer.",
      rec: false,
    },
  ];
      ...
      {userMode === "power" && (
        <div className="pt-6 border-t border-border">
          ...
          <div className="mt-6">
            <DecisionCard
              decision={
                engineDecisions.find(
                  (decision) => decision.key === "architecture-normalization",
                ) ?? engineDecisions[0]
              }
            />
          </div>
...
const stackSchema = z.object({
  languageVersion: z
    ...
    .regex(
      /^v?\d+(\.\d+)*(\.\d+)?$/,
      "Invalid version format (e.g. 18.0.0 or v20)",
    ),
});
...
export const StepStack = () => {
  ...
  const pkgManagers: Record<
    PlanConfig["stack"]["language"],
    PlanConfig["stack"]["packageManager"][]
  > = {
              ...
              onClick={() =>
                updateConfig({
                  stack: {
                    ...config.stack,
                    language: l.id,
                    framework: frameworks[l.id][0],
                    packageManager: pkgManagers[l.id][0],
                    builder: l.id === "TypeScript" ? "Vite" : "None",
                    rustMode:
                      l.id === "Rust" ? config.stack.rustMode : "template",
                  },
                })
              }
              className={cn(
                "flex flex-col items-center gap-2 min-w-[80px] p-3 rounded-xl border cursor-pointer transition-all",
                config.stack.language === l.id
        ...
        <div>
            ...
            <div className="mt-6 space-y-2">
              ...
              <div className="space-y-2">
                {[
                  ...
                  const disabled =
                    mode.id === "projen-experimental" && userMode !== "power";
                  return (
                    <div
                      key={mode.id}
                      onClick={() =>
                        ...
                        updateConfig({
                          stack: {
                            ...config.stack,
                            rustMode:
                              mode.id as PlanConfig["stack"]["rustMode"],
                          },
                        })
                      }
                      className={cn(
          ...
          <div>
            ...
            <Input
              ...
              placeholder={
                config.stack.language === "TypeScript" ? "20.0.0" : "1.20"
              }
              onChange={(e) => {
                register("languageVersion").onChange(e); // Trigger validation
                updateConfig({
                  stack: { ...config.stack, languageVersion: e.target.value },
                });
              }}
            />
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {pkgManagers[config.stack.language]?.map((pm) => (
                <Badge
                  key={pm}
                  variant={
                    config.stack.packageManager === pm ? "default" : "outline"
                  }
                  className="cursor-pointer"
                  onClick={() =>
                    updateConfig({
                      stack: { ...config.stack, packageManager: pm },
                    })
                  }
                >
                  {pm}
          ...
          <div>
            ...
            <div className="flex gap-2 bg-muted p-1 rounded-lg border border-border">
              {dependencyStrategies.map((strategy) => (
                <button
                  ...
                  onClick={() =>
                    updateConfig({
                      stack: { ...config.stack, dependencyStrategy: strategy },
                    })
                  }
                  className={cn(
            ...
            <p className="text-[10px] text-muted-foreground mt-1">
              {config.stack.dependencyStrategy === "semver"
                ? "Allow compatible updates (^1.2.3)"
                : "Lock to exact versions (1.2.3)"}
            </p>
          </div>
          ...
          <div>
            ...
            {config.stack.language === "TypeScript" ? (
              ...
              <>
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1">
                  Auto-selected based on framework
                </p>
              </>
            )}
          </div>
              ...
              <div className="flex gap-2 bg-muted p-1 rounded-lg border border-border">
                {LINTER_OPTIONS.map((l) => (
                  <button
                    ...
                    onClick={() =>
                      updateConfig({
                        quality: { ...config.quality, linter: l },
                      })
                    }
                    className={cn(
                      "flex-1 text-xs py-1.5 rounded-md transition-all",
                      config.quality.linter === l
                        ? "bg-background text-foreground shadow-sm"
                        : "text-muted-foreground hover:text-foreground",
                    )}
                  >
              ...
              <div className="flex gap-2 bg-muted p-1 rounded-lg border border-border">
                {FORMATTER_OPTIONS.map((f) => (
                  <button
                    ...
                    onClick={() =>
                      updateConfig({
                        quality: { ...config.quality, formatter: f },
                      })
                    }
                    className={cn(
                      "flex-1 text-xs py-1.5 rounded-md transition-all",
                      config.quality.formatter === f
                        ? "bg-background text-foreground shadow-sm"
                        : "text-muted-foreground hover:text-foreground",
                    )}
                  >
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
            ...
            <Switch
              ...
              onCheckedChange={(v) =>
                updateConfig({ quality: { ...config.quality, testing: v } })
              }
            />
          </div>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateConfig({
                    quality: {
                      ...config.quality,
                      testFramework: e.target.value as TestFramework,
                    },
                  })
                }
          ...
          <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
            ...
            <Switch
              ...
              onCheckedChange={(v) =>
                updateConfig({ quality: { ...config.quality, e2eTests: v } })
              }
            />
          </div>
        </div>
            ...
            onClick={() =>
              updateConfig({
                quality: {
                  ...config.quality,
                  qualityPlatform:
                    config.quality.qualityPlatform === "Trunk.io"
                      ? "None"
                      : "Trunk.io",
                },
              })
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                Unify your linters, formatters, and static analysis into a
                single check. Eliminates config drift and speeds up CI.
              </p>
            </div>
  ...
  const jobs = [
    {
      ...
      label: "Run Tests",
      icon: TestTube,
      desc: "Execute unit and integration tests on push",
    },
    {
      id: "buildArtifacts",
      label: "Build Artifacts",
      icon: Box,
      desc: "Compile code and store binaries/assets",
    },
    {
      id: "automaticRelease",
      label: "Semantic Release",
      icon: Rocket,
      desc: "Auto-version and publish based on commits",
    },
    {
      id: "deployToCloud",
      label: "Deploy Preview",
      icon: Cloud,
      desc: "Deploy to Vercel/Netlify/AWS on PR",
    },
  ];
        ...
        {jobs.map((job) => (
          <div
            ...
            className={cn(
              ...
              config.ci[job.id as keyof typeof config.ci]
                ? "bg-card border-zinc-600 shadow-md"
          ...
          >
            ...
            <Switch
              ...
              onCheckedChange={(v) =>
                updateConfig({ ci: { ...config.ci, [job.id]: v } })
              }
            />
          </div>
        ))}
      ...
      <div className="p-4 bg-muted/30 rounded-xl border border-dashed border-border">
        <div className="flex items-center gap-2 text-muted-foreground mb-2">
          ...
          <span className="text-xs font-mono font-semibold">
            .github/workflows/ci.yml Preview
          </span>
        </div>
    ...
    <div className="space-y-6 animate-in slide-in-from-right-8 duration-500">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Controls Dependabot grouping/schedule and CI matrix breadth.
            </p>
          </div>
          <Badge variant="outline" className="capitalize">
            {String(config.noiseBudget)}
          </Badge>
        </div>
        <div className="flex flex-wrap gap-2">
        ...
        {userMode === "power" && (
          <div>
            ...
            <Input
              ...
              max={100}
              value={
                typeof config.noiseBudget === "number" ? config.noiseBudget : 50
              }
              onChange={(e) =>
                updateConfig({ noiseBudget: Number(e.target.value) })
              }
            />
          </div>
        )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          <div className="flex justify-between items-start mb-3">
            ...
            />
            <Switch
              checked={config.security.codeScanning}
              onCheckedChange={(v) =>
                updateConfig({
                  security: { ...config.security, codeScanning: v },
                })
              }
            />
          </div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Find vulnerabilities and errors in your code using CodeQL.
          </p>
        </div>
        ...
        <div
          className={cn(
            "p-4 rounded-xl border transition-all",
            config.security.secretScanning
        ...
        >
          <div className="flex justify-between items-start mb-3">
            ...
            />
            <Switch
              checked={config.security.secretScanning}
              onCheckedChange={(v) =>
                updateConfig({
                  security: { ...config.security, secretScanning: v },
                })
              }
            />
          </div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Prevent fraudulent use of secrets committed accidentally.
          </p>
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl border border-border bg-card">
        <div className="flex items-center justify-between mb-4">
          ...
          <Switch
            checked={config.security.dependencyUpdates}
            onCheckedChange={(v) =>
              updateConfig({
                security: { ...config.security, dependencyUpdates: v },
              })
            }
          />
        </div>
            ...
            <div className="flex gap-2">
              {DEPENDENCY_FREQUENCIES.map((freq) => (
                <Badge
                  key={freq}
                  variant={
                    config.security.dependencyUpdateFrequency === freq
                      ? "default"
                      : "outline"
                  }
                  className="cursor-pointer capitalize"
                  onClick={() =>
                    updateConfig({
                      security: {
                        ...config.security,
                        dependencyUpdateFrequency: freq,
                      },
                    })
                  }
                >
      ...
      <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Scaffold .env.example and .gitignore rules.
            </p>
          </div>
        </div>
        <Switch
          checked={config.security.manageEnv}
          onCheckedChange={(v) =>
            updateConfig({ security: { ...config.security, manageEnv: v } })
          }
        />
      </div>
    </div>
...
export const StepGitHub = () => {
    ...
    if (e.key === "Enter" && newTopic.trim()) {
      if (!config.github.topics.includes(newTopic.trim())) {
        updateConfig({
          github: {
            ...config.github,
            topics: [...config.github.topics, newTopic.trim()],
          },
        });
      }
      setNewTopic("");
  ...
  const removeTopic = (t: string) => {
    updateConfig({
      github: {
        ...config.github,
        topics: config.github.topics.filter((topic) => topic !== t),
      },
    });
  };
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      updateConfig({ github: { ...config.github, copilot: v } })
                    }
                  />
                </div>

                <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mt-6 mb-3">
                  Pull Requests
                </h4>
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v) =>
                        updateConfig({
                          github: {
                            ...config.github,
                            pr: { ...config.github.pr, allowSquashMerge: v },
                          },
                        })
                      }
                    />
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v) =>
                        updateConfig({
                          github: {
                            ...config.github,
                            pr: { ...config.github.pr, deleteBranchOnMerge: v },
                          },
                        })
                      }
                    />
            ...
            <TabsContent value="rules" className="mt-0 space-y-6">
              <div className="space-y-4">
                <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded-lg text-xs text-blue-500 flex gap-2">
                  ...
                  Using GitHub Rulesets for enhanced protection on the default
                  branch.
                </div>
                ...
                <div className="p-3 rounded-lg border border-border bg-card">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {GOVERNANCE_POSTURES.map((posture) => (
                      <Badge
                        key={posture}
                        variant={
                          config.governancePosture === posture
                            ? "default"
                            : "outline"
                        }
                        className="cursor-pointer"
                        onClick={() =>
                          updateConfig({ governancePosture: posture })
                        }
                      >
                        {posture}
                      </Badge>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-2">
                    {userMode === "power"
                      ? "Power mode lets you override detailed branch/ruleset knobs below."
                      : "Simple mode applies compiler-driven branch rules, status checks, and security defaults."}
                  </p>
                </div>
                ...
                {userMode === "power" ? (
                            ...
                            updateConfig({
                              github: {
                                ...
                                branches: {
                                  ...config.github.branches,
                                  protection: {
                                    ...config.github.branches.protection,
                                    requirePr: v,
                                  },
                                },
                              },
                  ...
                  <div className="p-3 rounded-lg border border-dashed border-border text-xs text-muted-foreground">
                    Governance posture controls default branch protections in
                    Simple mode. Switch to Power mode to fine-tune each rule.
                  </div>
                )}
              </div>
            </TabsContent>
                  ...
                  {ACTION_PERMISSION_OPTIONS.map((p) => (
                    <div
                      ...
                      className={cn(
                        ...
                        config.github.actions.permissions === p
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div
                    ...
                    className={cn(
                      ...
                      config.github.actions.runners === "github"
                  ...
                  <div
                    ...
                    className={cn(
                      "p-3 border rounded-lg cursor-pointer flex items-center justify-center gap-2 text-xs font-medium transition-all",
                      config.github.actions.runners === "self-hosted"
                        ? "bg-accent text-foreground"
                        : "bg-card text-muted-foreground border-border",
                    )}
                  >
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      updateConfig({
                        github: {
                          ...config.github,
                          actions: { ...config.github.actions, allowPr: v },
                        },
                      })
                    }
                  />
                    ...
                    onClick={() => {
                      ...
                      if (url) {
                        ...
                        updateConfig({
                          github: {
                            ...config.github,
                            webhooks: [...config.github.webhooks, newHook],
                          },
                        });
                      }
                    }}
                        ...
                        <Button
                          ...
                          onClick={() =>
                            updateConfig({
                              github: {
                                ...config.github,
                                webhooks: config.github.webhooks.filter(
                                  (h) => h.id !== hook.id,
                                ),
                              },
                            })
                  ...
                  {["production", "staging", "development"].map((env) => (
                    <div
                      ...
                      className={cn(
                        ...
                        config.github.environments.includes(env)
                      ...
                      <Switch
                        ...
                        onCheckedChange={(v) => {
                          const newEnvs = v
                            ...
                            : config.github.environments.filter(
                                (e) => e !== env,
                              );
                          updateConfig({
                            github: { ...config.github, environments: newEnvs },
                          });
                        }}
                      />
                    </div>
            ...
            <TabsContent value="secrets" className="mt-0 space-y-6">
              <div className="space-y-4">
                <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg text-xs text-amber-500 mb-4">
                  <p>
                    We cannot set actual secret values. Defined secrets will be
                    listed as requirements in the generated plan and README.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {config.github.secrets.map((secret) => (
                    <Badge
                      key={secret}
                      variant="outline"
                      className="px-3 py-1 flex gap-2 items-center bg-card border-border"
                    >
                      {secret}
                      <button
                        onClick={() =>
                          updateConfig({
                            github: {
                              ...config.github,
                              secrets: config.github.secrets.filter(
                                (s) => s !== secret,
                              ),
                            },
                          })
...
export const StepDocs = () => {
  ...
  const standardFiles = [
    ...
    },
    {
      ...
      desc: "Guidelines for contributors",
      icon: Users,
    },
    {
      id: "codeowners",
      label: "CODEOWNERS",
      desc: "Define who is responsible for code",
      icon: Shield,
    },
    {
      id: "pullRequestTemplate",
      label: "PR Template",
      desc: "Standardize Pull Request descriptions",
      icon: GitPullRequest,
    },
    {
      id: "issueTemplates",
      label: "Issue Templates",
      desc: "Standardize bug reports & feature requests",
      icon: ListTodo,
    },
    {
      id: "adr",
  ...
  const styles: { id: DocStyle; label: string; desc: string }[] = [
    { id: "none", label: "None", desc: "No specific style enforced" },
    {
      id: "diataxis",
      label: "Diataxis",
      desc: "Tutorials, How-To, Reference, Explanation",
    },
    {
      id: "microsoft",
      label: "Microsoft",
      desc: "Warm, conversational, and precise",
    },
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-right-8 duration-500">
      <div>
        <Label className="mb-4 block text-muted-foreground">
          Standard Files
        </Label>
          ...
          {standardFiles.map((file) => {
            ...
            return (
              <div
                ...
                onClick={() =>
                  updateConfig({
                    docs: { ...config.docs, [file.id]: !isActive },
                  })
                }
                className={cn(
                  "p-3 border rounded-lg cursor-pointer transition-all flex items-start gap-3",
                  isActive
              ...
              >
                <div
                  className={cn(
                    "p-1.5 rounded bg-muted border border-border",
          ...
          <div className="grid grid-cols-4 gap-3">
            {frameworks.map((fw) => (
              <div
                key={fw.id}
                onClick={() =>
                  updateConfig({ docs: { ...config.docs, framework: fw.id } })
                }
                className={cn(
          ...
          <div className="space-y-4 animate-in slide-in-from-top-2">
            <div className="flex items-center justify-between p-3 border rounded-lg bg-card border-border">
              ...
              <Switch
                ...
                onCheckedChange={(v) =>
                  updateConfig({ docs: { ...config.docs, deployToPages: v } })
                }
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {styles.map((style) => (
              <div
                ...
                onClick={() =>
                  updateConfig({
                    docs: { ...config.docs, styleGuide: style.id },
                  })
                }
                className={cn(
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          label: "Testing",
          value: config.quality.testing
            ? config.quality.testFramework
            : "Disabled",
        },
    ...
    },
    {
      ...
      items: [
        {
          label: "Scanning",
          value: config.security.codeScanning ? "Enabled" : "Disabled",
        },
        {
          label: "Secrets",
          value: config.security.secretScanning ? "Enabled" : "Disabled",
        },
        {
          label: "Updates",
          value: config.security.dependencyUpdates
            ? config.security.dependencyUpdateFrequency
            : "Disabled",
        },
        {
          label: ".env Strategy",
          value: config.security.manageEnv ? "Gitignore" : "Manual",
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          label: "Copilot",
          value: config.github.copilot ? "Enabled" : "Disabled",
        },
      ],
    },
  ];
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-8 duration-500">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground text-sm">
          Review your configuration before generating the repository plan.
        </p>
      </div>
