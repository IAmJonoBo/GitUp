  ...
  if (node.type === "folder") {
    return (
      <div>
        <div
          className={cn(
            "flex items-center gap-2 py-1 px-2 cursor-pointer text-xs",
            isSelected
...
export const RepoTree = () => {
  ...
  const selectedContent = useMemo(() => {
    ...
    const sourceOperation = changePlan.operations.find(
      (operation) => operation.target === selectedPath,
    );
    ...
    return [
      ...
      "",
      sourceOperation
        ? `Planned action: ${sourceOperation.message}`
        : "Planned action: generated from RepoSpec file list.",
    ].join("\n");
  }, [
    changePlan.operations,
    repoSpec.architecture,
    repoSpec.packageManager,
    selectedPath,
  ]);
        ...
        <div className="w-5/12 overflow-y-auto border-r border-border py-2 scrollbar-thin bg-muted/10">
          {treeData.map((node) => (
            <TreeItem
              ...
              node={node}
              depth={0}
              onSelect={(nodeValue) => setSelectedPath(nodeValue.path)}
              selectedPath={selectedPath}
            />
          ))}
        </div>
            ...
            <div className="min-h-full p-4">
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    type="button"
                    className="w-full text-left bg-transparent p-0 border-0"
                  >
                    <pre className="text-[10px] leading-relaxed font-mono text-foreground/80 whitespace-pre-wrap">
                      {selectedContent}
                    </pre>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top">
                  Derived from RepoSpec + ChangePlan
                </TooltipContent>
              </Tooltip>
            </div>
          ) : (
