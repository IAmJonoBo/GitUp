...
export const DiffInterstitial = () => {
  ...
  if (
    !pendingDiff ||
    (!pendingDiff.added.length && !pendingDiff.removed.length)
  ) {
    return null;
  }
    ...
    <div className="mb-6">
      <Card className="border-amber-500/30 bg-amber-500/5 p-4 space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm text-amber-200 font-semibold flex items-center gap-2">
              <ArrowRightLeft className="w-4 h-4" /> Review plan changes before
              continuing
            </p>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <p className="text-xs uppercase text-emerald-300 mb-2">
              Added operations
            </p>
          ...
          <div>
            <p className="text-xs uppercase text-rose-300 mb-2">
              Removed operations
            </p>
            <ul className="text-xs text-zinc-300 space-y-1">
