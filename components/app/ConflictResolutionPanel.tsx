...
const renderEffectDiff = (
  ...
  return (
    <div>
      <h5 className="text-xs uppercase tracking-wide text-zinc-500 mb-2">
        {label}
      </h5>
          ...
          {leftEntries.length === 0 ? (
            ...
            leftEntries
              ...
              .map(([key, value]) => (
                <div
                  key={key}
                  className="text-xs text-zinc-200 flex justify-between gap-2"
                >
                  <code>{key}</code>
...
export const ConflictResolutionPanel = () => {
  ...
  const resolveCapabilityConflict = useStore(
    (state) => state.resolveCapabilityConflict,
  );
  const capabilityOwnerOverrides = useStore(
    (state) => state.capabilityOwnerOverrides,
  );
    ...
    <Card className="bg-zinc-950 border border-amber-500/20 p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Badge variant="outline" className="border-amber-500/30 text-amber-300">
          {conflicts.length} collision{conflicts.length === 1 ? "" : "s"}
        </Badge>
      </div>
      ...
      {conflicts.map((conflict) => {
        ...
        const activeOwner = ownerIsOverridden
          ? conflict.challenger
          : conflict.owner;
        const inactive = ownerIsOverridden
          ? conflict.owner
          : conflict.challenger;

        return (
          <div
            key={`${conflict.capability}-${conflict.challenger.packId}`}
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Single-owner capability; pick one pack to keep ownership.
                </p>
              </div>
              <div className="flex gap-2">
                ...
                >
                  Owner: {activeOwner.packId}
                </Badge>
                <Badge
                  variant="outline"
                  className="border-zinc-700 text-zinc-300"
                >
                  Priority {activeOwner.priority}
                </Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="rounded-lg border border-zinc-700 bg-zinc-900/60 p-3">
                <p className="text-xs uppercase tracking-wide text-zinc-300 mb-2">
                  Alternative ({inactive.packId})
                </p>
                <Button
                  size="sm"
                  variant="ghost"
            ...
            <div className="space-y-2">
              {renderEffectDiff(
                "Scripts",
                activeOwner.effects.scripts,
                inactive.effects.scripts,
              )}
              {renderEffectDiff(
                "Dependencies",
                activeOwner.effects.dependencies,
                inactive.effects.dependencies,
              )}
              {renderEffectDiff(
                "Dev dependencies",
                activeOwner.effects.devDependencies,
                inactive.effects.devDependencies,
              )}
            </div>
