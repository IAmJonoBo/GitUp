...
export const buildChangePlanDiff = (previous: ChangePlan, next: ChangePlan): ChangePlanDiff => {
  const previousFingerprints = new Set(previous.operations.map(operationFingerprint));
  const nextFingerprints = new Set(next.operations.map(operationFingerprint));

  return {
    added: next.operations.filter((operation) => !previousFingerprints.has(operationFingerprint(operation))),
    removed: previous.operations.filter((operation) => !nextFingerprints.has(operationFingerprint(operation))),
  };
};
...
export const createEngineDecisionPayloads = (
  ...
  const securityRecommendation = designSpec.security.codeScanning && designSpec.security.secretScanning
  ...
  return [
    {
      ...
      tradeOffs: ['Higher initial scaffolding effort', 'Requires team alignment on folder contracts'],
      alternatives: ['Standard', 'Vertical Slice', 'MVC'].filter((candidate) => candidate !== designSpec.architecture),
      confidence: 'High',
    },
    {
      ...
      tradeOffs: ['Switching package manager later can invalidate lockfiles', 'Framework-specific conventions reduce portability'],
    ...
    },
    {
      ...
      tradeOffs: ['More guardrails may slow first merge', 'Automation requires permissions upfront'],
