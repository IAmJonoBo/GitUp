...
describe('declared pack conflicts', () => {
  ...
  it('uses stable id sort as deterministic tie-breaker', () => {
    PACK_REGISTRY.push(
      { ...extraPacks[0], priority: 40, id: 'pack.test.same-b', conflicts: ['pack.test.same-a'], capabilities: [{ name: 'test:same-b' }] },
      { ...extraPacks[1], priority: 40, id: 'pack.test.same-a', conflicts: ['pack.test.same-b'], capabilities: [{ name: 'test:same-a' }] },
    );
