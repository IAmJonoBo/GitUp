...
import { WizardState, Language, ProjectType } from '@repoforge/shared';
import { PROJECT_TYPES, LANGUAGES, LICENSES, TECH_OPTIONS, LANGUAGE_ICONS, CODES_OF_CONDUCT, PACKAGE_MANAGERS, RECIPES } from '@repoforge/shared';
import { StepContainer, Tooltip } from './ui/Layouts';
import { hostBridge } from '../transport/hostBridge';
import { Sparkles, Check, Info, Server, Layout, Database, Terminal, Code2, MessageSquareText, Shield, Users, BookOpen, GitBranch, Lock, Globe, FileJson, Package, Zap } from 'lucide-react';

interface StepProps {
  state: WizardState;
  updateState: (updates: Partial<WizardState>) => void;
  updateNestedState: (category: keyof WizardState, updates: any) => void;
}

// --- Step 1: Basics ---
export const StepBasics: React.FC<StepProps> = ({ state, updateNestedState }) => {
  return (
    <StepContainer title="Project Identity" description="Define the core metadata and visibility of your repository.">
      <div className="grid gap-6">

        {/* Name & Desc */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-300 flex items-center gap-2">Repository Name</label>
            <input
              type="text"
              value={state.projectDetails.name}
              onChange={(e) => updateNestedState('projectDetails', { ...state.projectDetails, name: e.target.value })}
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 bg-slate-900 p-1 rounded-lg border border-slate-700">
                {['public', 'private'].map((vis) => (
                    <button
                        key={vis}
                        onClick={() => updateNestedState('projectDetails', { ...state.projectDetails, visibility: vis })}
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            onChange={(e) => updateNestedState('projectDetails', { ...state.projectDetails, description: e.target.value })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-2">
            ...
            <select
               ...
               onChange={(e) => updateNestedState('projectDetails', { ...state.projectDetails, type: e.target.value })}
          ...
          <div className="space-y-2">
             ...
             <select
              ...
              onChange={(e) => updateNestedState('projectDetails', { ...state.projectDetails, license: e.target.value })}
              className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-brand-500 outline-none"
            >
          ...
          <div className="space-y-2">
             ...
             <div className="relative">
                ...
                <input
                    ...
                    onChange={(e) => updateNestedState('projectDetails', { ...state.projectDetails, defaultBranch: e.target.value })}
...
// --- Step 2: Tech Stack ---
export const StepStack: React.FC<StepProps> = ({ state, updateNestedState, updateState }) => {
  ...
  useEffect(() => {
    if (state.techStack.language) {
      ...
      if (defaultPkg && !state.techStack.packageManager) {
        updateNestedState('techStack', { ...state.techStack, packageManager: defaultPkg });
      }
  ...
  }, [state.techStack.language, state.projectDetails.type]);
  ...
  const applyRecipe = (recipeId: string) => {
      ...
      const newTechStack = { ...state.techStack, ...recipe.config };
      // Filter out automation keys from techStack if any leaked (Recipe config has both)
      // Actually Recipe config is Partial<TechStack & Automation>

      const { ci, docker, linting, ...techProps } = recipe.config as any;

      updateNestedState('techStack', { ...state.techStack, ...techProps });
      updateNestedState('automation', { ...state.automation, ci: ci ?? state.automation.ci, docker: docker ?? state.automation.docker, linting: linting ?? state.automation.linting });
  };

  const filteredOptions = TECH_OPTIONS.filter(opt =>
    !opt.recommendedFor || opt.recommendedFor.includes(state.techStack.language)
  );

  return (
    <StepContainer title="Tech Stack & Context" description="Choose the engine for your application and guide the AI.">
      <div className="grid gap-8">
                ...
                {RECIPES.map(recipe => (
                    ...
                    >
                         <div className="flex items-center gap-2 mb-2">
                             ...
                             <span className="font-semibold text-slate-200 group-hover:text-yellow-400 transition-colors">{recipe.name}</span>
                         </div>
        ...
        <div className="space-y-4">
            <h3 className="text-white font-medium flex items-center gap-2"><Code2 size={18}/> Primary Language</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {LANGUAGES.map((lang) => (
                <button
                key={lang}
                onClick={() => updateNestedState('techStack', {
                    ...state.techStack,
                    language: lang,
                    frameworks: [],
                ...
                >
                    <i className={`${LANGUAGE_ICONS[lang]} text-4xl mb-1 group-hover:scale-110 transition-transform`}></i>
                    <span className={`text-sm font-medium ${state.techStack.language === lang ? 'text-white' : 'text-slate-400'}`}>{lang}</span>
                    {state.techStack.language === lang && (
            ...
            <div className="space-y-3 animate-fade-in">
                ...
                <div className="flex flex-wrap gap-2">
                    {PACKAGE_MANAGERS[state.techStack.language]?.map((pm) => (
                        <button
                            key={pm}
                            onClick={() => updateNestedState('techStack', { ...state.techStack, packageManager: pm })}
        ...
        {/* Frameworks */}
        <div className="space-y-3">
            <h3 className="text-white font-medium flex items-center gap-2"><Layout size={18}/> Frameworks & Tools</h3>
                        ...
                        >
                            ...
                            <div className="flex-1">
                                <div className="flex items-center justify-between mb-1">
                                    <span className={`font-medium ${state.techStack.frameworks.includes(opt.id) ? 'text-brand-300' : 'text-slate-300'}`}>
                                        {opt.name}
                                    </span>
                                    {state.techStack.frameworks.includes(opt.id) && <Check size={16} className="text-brand-400" />}
                                </div>
        ...
        <div className="space-y-3">
            ...
            <div className="relative group">
                <textarea
                    ...
                    onChange={(e) => updateNestedState('projectDetails', { ...state.projectDetails, aiPrompt: e.target.value })}
...
export const StepGovernance: React.FC<StepProps> = ({ state, updateNestedState }) => {
    return (
        <StepContainer title="Community & Governance" description="Set the rules of engagement and templates for your contributors.">
            <div className="grid gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <select
                        ...
                        onChange={(e) => updateNestedState('governance', { ...state.governance, codeOfConduct: e.target.value })}
                        className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-brand-500 outline-none"
                    >
                        {CODES_OF_CONDUCT.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                    </select>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div
                        onClick={() => updateNestedState('governance', { ...state.governance, contributionGuide: !state.governance.contributionGuide })}
                    ...
                    >
                        <div className={`mt-1 p-1.5 rounded ${state.governance.contributionGuide ? 'bg-brand-500 text-white' : 'bg-slate-800 text-slate-500'}`}>
                            <Users size={16} />
                        </div>
                        <div>
                            <h4 className={`text-sm font-semibold ${state.governance.contributionGuide ? 'text-brand-100' : 'text-slate-300'}`}>CONTRIBUTING.md</h4>
                            <p className="text-xs text-slate-500 mt-1">Add guidelines for how people can help.</p>
                        </div>
                    </div>

                    <div
                        onClick={() => updateNestedState('governance', { ...state.governance, issueTemplates: !state.governance.issueTemplates })}
                    ...
                    >
                         <div className={`mt-1 p-1.5 rounded ${state.governance.issueTemplates ? 'bg-brand-500 text-white' : 'bg-slate-800 text-slate-500'}`}>
                            <FileJson size={16} />
                        </div>
                         <div>
                            <h4 className={`text-sm font-semibold ${state.governance.issueTemplates ? 'text-brand-100' : 'text-slate-300'}`}>Issue Templates</h4>
                            <p className="text-xs text-slate-500 mt-1">Pre-defined forms for bug reports & features.</p>
                        </div>
                    </div>

                    <div
                        onClick={() => updateNestedState('governance', { ...state.governance, pullRequestTemplate: !state.governance.pullRequestTemplate })}
                    ...
                    >
                         <div className={`mt-1 p-1.5 rounded ${state.governance.pullRequestTemplate ? 'bg-brand-500 text-white' : 'bg-slate-800 text-slate-500'}`}>
                            <GitBranch size={16} />
                        </div>
                         <div>
                            <h4 className={`text-sm font-semibold ${state.governance.pullRequestTemplate ? 'text-brand-100' : 'text-slate-300'}`}>PR Template</h4>
...
export const StepAutomation: React.FC<StepProps> = ({ state, updateNestedState }) => {
    ...
    const FeatureToggle = ({ icon: Icon, title, desc, active, onClick, badge }: any) => (
        <div
            onClick={onClick}
        ...
        >
            <div className="flex items-start gap-4">
                <div className={`p-2 rounded-lg transition-colors ${active ? 'bg-brand-500/20 text-brand-300' : 'bg-slate-800 text-slate-500 group-hover:bg-slate-700'}`}>
                    <Icon size={24} />
                </div>
                <div className="flex-1">
                    <div className="flex items-center justify-between">
                        <h4 className={`font-medium ${active ? 'text-white' : 'text-slate-300'}`}>
                            {title}
                            {badge && <span className="ml-2 text-[10px] uppercase tracking-wider bg-purple-500/20 text-purple-300 px-1.5 py-0.5 rounded border border-purple-500/30">{badge}</span>}
                        </h4>
                    ...
                    <p className="text-sm text-slate-500 mt-1 group-hover:text-slate-400 transition-colors">{desc}</p>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <StepContainer title="DevOps & Automation" description="Configure the robots that will maintain your code.">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FeatureToggle
                    icon={Server}
                    ...
                    onClick={() => updateNestedState('automation', { ...state.automation, ci: !state.automation.ci })}
                />
                <FeatureToggle
                    icon={Database}
                    title="Docker"
                    ...
                    onClick={() => updateNestedState('automation', { ...state.automation, docker: !state.automation.docker })}
                />
                 <FeatureToggle
                    icon={Lock}
                    title="Dependabot"
                    ...
                    onClick={() => updateNestedState('automation', { ...state.automation, dependabot: !state.automation.dependabot })}
                    badge="Security"
                />
                 <FeatureToggle
                    icon={GitBranch}
                    ...
                    onClick={() => updateNestedState('automation', { ...state.automation, husky: !state.automation.husky })}
                />
                <FeatureToggle
                    icon={BookOpen}
                    ...
                    onClick={() => updateNestedState('automation', { ...state.automation, docs: !state.automation.docs })}
                />
                <FeatureToggle
                    icon={Terminal}
                    ...
                    onClick={() => updateNestedState('automation', { ...state.automation, linting: !state.automation.linting })}
                />
            </div>
        </StepContainer>
    );
